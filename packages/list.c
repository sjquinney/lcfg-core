...
LCFGChange lcfgpkglist_merge_package( LCFGPackageList * pkglist,
  ...
  const char * match_arch = LCFG_PKG_WILDCARD;
  ...
  if ( merge_rules&LCFG_MERGE_RULE_USE_PREFIX ) {
    ...
    char cur_prefix = cur_pkg != NULL ? cur_pkg->prefix : LCFG_PKG_PREFIX_NONE;
    char new_prefix = new_pkg->prefix;

    if ( new_prefix != LCFG_PKG_PREFIX_NONE ) {

      switch(new_prefix)
	{
	case LCFG_PKG_PREFIX_REMOVE:
	  remove_old = true;
	  accept     = true;
	  break;
	case LCFG_PKG_PREFIX_ADD:
	case LCFG_PKG_PREFIX_PIN:
	  remove_old = true;
	  append_new = true;
	  accept     = true;
	  break;
	case LCFG_PKG_PREFIX_ANY:
	  if ( cur_pkg == NULL ) {
            append_new = true;
          }
	  accept = true;
	  break;
	case LCFG_PKG_PREFIX_UPDATE:
	  if ( cur_pkg != NULL && cur_prefix != LCFG_PKG_PREFIX_PIN ) {
	    remove_old = true;
	    append_new = true;
	  }
	  accept = true;
	  break;
        case LCFG_PKG_PREFIX_MIN:
          if ( cur_pkg == NULL ||
	...
	default:
	  *msg = lcfgpackage_build_message( new_pkg,
      ...
      if ( remove_old && cur_pkg != NULL ) {
        if ( cur_prefix == LCFG_PKG_PREFIX_PIN ) {
          remove_old = false;
          append_new = false;
          accept = false;
 ...
 *
 * This creates a new @c LCFGPackageList from the contents of a Debian
