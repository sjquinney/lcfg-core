 ...
 * for components in the @c LCFGComponentList which have 'ngeneric'
...
LCFGTagList * lcfgcomplist_ngeneric_components( const LCFGComponentList * complist ) {

  LCFGTagList * comp_names = lcfgtaglist_new();

  if ( !lcfgcomplist_is_empty(complist) ) return comp_names;
...
char * lcfgcomplist_signature( const LCFGComponentList * complist ) {
  ...
  /* The '=' and '\n' characters are hashed so that the signature
     matches with that generated by the old client. At some point we
     should revisit this whole thing... */
